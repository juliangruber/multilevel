...
Db.prototype.sublevel = function (name) {
  if (!this.sublevels || !this.sublevels[name]) {
    addSubLevel(this, this, name, {});
  }
  return this.sublevels[name];
};

function addSubLevel (self, db, name, manifest) {
  db.sublevels = db.sublevels || {};
  db.sublevels[name] = new Db(manifest);
  self._buildAll(manifest, db.sublevels[name], db._path.concat(name), db);
}
...
Db.prototype._buildAll = function (_db, db, path, parent) {
  ...
  for (var name in _db.sublevels) {
    addSubLevel(self, db, name, _db.sublevels[name]);
  }
};
