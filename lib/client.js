...
return function (db) {
  // Manifest contains methods and sublevels
  var m = manifest(db || {methods: {}});

  // Hide MuxDemux's methods through duplexer.
  var mdm = MuxDemux();
  var db = duplexer(mdm, mdm);

  db.isClient = true;
  db.methods = m.methods;

  // Rpc
  var client = db.rpc = rpc(null, true);
  client.pipe(mdm.createStream('rpc')).pipe(client);

  // isClosed & isOpen tell you whether you are currently connected
  // db.close() disconnects
  var isOpen = true;
  db.isOpen = function () { return isOpen };
  db.isClosed = function () { return !isOpen };

  db.destroy = function () { db.close() };
  ...
  (function buildAll (db, api, path, parent) {
    ...
    api.sublevel = function (name) {
      if (api.sublevels[name]) return api.sublevels[name];
